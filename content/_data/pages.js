...
if (domainName.endsWith("wordpress.com")) {
  wordpressAPI = 'https://public-api.wordpress.com/wp/v2/sites/' + domainName + '/pages?orderby=date&order=desc&per_page=100';
} else {
  wordpressAPI = 'https://' + domainName + '/wp-json/wp/v2/pages?orderby=date&order=desc&per_page=100';
}
...
async function wpPostPages() {
  ...
  try {
    ...
    console.log("Found " + res.headers.get('X-WP-TotalPages') + " pages");
